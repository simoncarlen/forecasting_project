...
\label{sec:DL models}
The most basic type of neural network model is the so-called feedforward neural network (ffNN) \cite{Chollet2017}. (Sometimes, this type of model is also called a densely connected neural network, or simply, a "dense" model). A ffNN essentially has the structure of the neural network model described above, i.e., with nodes in layers stacked sequentially, and where at each layer, a slightly more abstract representation of the inputs are produced by non-linear transformations \cite{LeCun2015}. With several such transformations, complex relationships in the data can often be learned \cite{LeCun2015, smlbook}.

For data where order matters (like with time series data), so-called recurrent neural network (RNN) models are often more appropriate than ffNN's \cite{LeCun2015}. RNN's can utilize past information in a sequence through a so-called 'state vector', which works like a memory, holding the information from previous time steps. The state vector is also continuously updated as a data sequence is being processed, so the output at any time $t$ depends on the current input as well as the information contained in the "memory" (or the systems hidden state) \cite{LeCun2015}. The most basic RNN architecture, the simple RNN model, can be difficult to train, however, and typically also performs poorly when processing longer sequences due to the way the state vector is maintained and updated \cite{LeCun2015}. For this reason, a much more common RNN model is the so-called long short-term memory (LSTM) model. The LSTM model utilizes three so-called gates (the input gate, the output gate, and the forget gate), which together with an additional vector (the cell-state vector), controls the information flow through the units of the network \cite{LeCun2015}. Yet another common RNN model is the gated recurrent unit (GRU) model, which is similar to the LSTM model but with fewer parameters \cite{Chollet2017}. 

Both types of neural network models described above, i.e., the dense as well as the RNN models, have been used to forecast e.g.\ PM$_{10}$ \cite{Arsov2021}, NO$_x$/NO$_2$ \cite{Rahimi2017, Gilik2021}, and O$_3$ and SO$_2$ \cite{Gilik2021}, many times with better results compared to linear models used on the same data \cite{Arsov2021, Rahimi2017}. Specifically, the RNN models (especially the LSTM model), with their ability to incorporate data from previous time steps have shown a promising potential when forecasting a wide range of pollutants at several different forecast horizons \cite{Arsov2021, Liao2020}.

\subsubsection{Training and fitting artificial neural networks}
The principal method for finding the optimal set of parameters/weights when the cost function is non-convex (as is the case with neural networks) is the gradient descent algorithm \cite{smlbook}. This algorithm iteratively updates the parameter vector by, at each iteration, adjusting the parameters by a small amount in the opposite direction to the gradient of the cost function \cite{smlbook}. How much the parameters are adjusted is decided by a hyperparameter called the learning rate. As mentioned, the gradient descent algorithm utilizes the gradient of the cost function, which is calculated with the so-called backpropagation method \cite{smlbook}. For large datasets, computing the gradient of the cost function can be problematic, which is why the stochastic gradient descent (SGD) algorithm typically is used instead \cite{smlbook}. SGD is very similar to gradient descent, except that subsamples of data are taken for every iteration instead of the entire dataset. Due to this, the learning rate cannot be constant but instead has to be adapted during training \cite{smlbook}. There are many extensions of SGD that includes such adaptations, like for example the root mean square propagation (RMSprop) and adaptive moments (Adam) methods \cite{smlbook}.
%Also with SGD, due to the gradient being estimated (with only part of the data), the learning rate cannot be constant but instead has to be adapted to gradually decrease \cite{smlbook}, 

Another important aspect of training neural networks concerns overfitting (something neural networks are highly prone to due to the many trainable parameters) \cite{smlbook}. To prevent overfitting, a technique called dropout is commonly used \cite{smlbook}. Dropout randomly selects units in the network and removes them, thereby also removing all connections to those units. The main idea behind dropout is to make the network fit a little less well to the training data, but as a result of that, generalize better to new data \cite{smlbook}. Depending on the type of network, dropout is typically implemented differently. For example, with ffNN's, the units dropped (the so-called dropout mask) at each layer is random, whereas for a RNN, the dropout mask should follow the same pattern at every timestep (this is called recurrent dropout) \cite{Chollet2017}. 

%Dropout will look different depending on the type of network, e.g., for a dense model, dropout is applied after the hidden layers, thereby randomly drop affecting the outputs of those layers, whereas for a RNN,   

%There are many choices as how to construct a neural network. For example, the number of units in each layer, how many layers to use, learning rate, and dropout rate are decisions up to the designer of the network.  

%The value of the cost function and its gradient is computed by the co-called backpropagation algorithm,  \cite{smlbook}. 
...
\subsubsection{Tests for autocorrelation in the forecast errors}
...
For air pollution forecasts, mechanistic models are commonly used \cite{ElHarbawi2013}. SLB-analys, for example, use a Gaussian dispersion model together with a street pollution model (OSPM) in their forecasting system for air pollution and pollen \cite{slbanalys}. As described in the preceeding sections, while statistical and machine learning models are also frequently used \cite{atmos7020015}, there are few publications where this approach is taken in Sweden or other Nordic countries; some examples include the paper from 2007 by Raimondo et al.\ \cite{Raimondo2007}, and the article by Staffogia et al. \cite{Stafoggia2020} from 2020. Consequently, a regression approach where air pollution is modeled as a time series under Swedish/Nordic weather and pollution conditions is currently lacking, and this is where this work aims to contribute. 
%While being able to give detailed descriptions of the dispersion of air pollution over e.g.\ a neighbourhood, mechanistic models require many factors for simulation and modeling \cite{ElHarbawi2013, Liao2020}. In contrast, statistical and machine learning models 
