...
\subsection{More complex regression techniques and deep learning}
For air pollution, more versatile and flexible regression techniques tend to give better forecasting results than the more simplistic linear models \cite{atmos7020015}. Some examples include regression trees, support vector regression (SVR), and artificial neural networks \cite{atmos7020015, FaganeliPucer2018}. In many ways, these regression techniques are extensions to the linear model, e.g., SVR is built on the idea of constructing non-linear transformations of the regressors, whereas the building blocks of artificial neural networks are generalized linear regression models \cite{smlbook}. In this work, a number of deep neural network architectures are utilized, and a deeper dive into the theory behind artificial neural networks is warranted. 

\subsubsection{Artificial neural network models}
The fundamental building block of an artificial neural network is what is often called the perceptron (or artificial neuron) \cite{smlbook}. In an artificial neuron, the output of a linear regression model is passed through a non-linear activation function; common choices for activation functions include the rectified linear unit (ReLU), or the tanh function:
...
\setlength{\jot}{10pt}
    ...
    \end{aligned}
\end{equation}
A collection of connected artificial neurons is what constitutes a layer in a neural network, and a network with many such layers stacked sequentially is what is referred to as a deep neural network \cite{smlbook}. When a set of input data is taken and passed through the layers of a neural network, each layer produces a slightly more abstract representation of its input by non-linear transformations, and with several such transformations, complex relationships in the data can be learned \cite{LeCun2015}.
...
where $\hat{y}_t (t-1)$ is the forecasted value of $y_t$ made one time step back (i.e.\ at time $t-1$)\cite{Montgomery2015}. Accuracy metrics such as the mean error (ME), the mean absolute error (MAE), the mean squared error (MSE), and the mean absolute percent forecast error (MAPE) are often utilized \cite{Montgomery2015}. These performance metrics are standard in regression evaluation, and the equations will not be repeated here. It can be noted though that the ME is an estimate of the expected value of $e_t(1)$, and strong departures from zero would indicate bias in the forecasts \cite{Montgomery2015}. Also, commonly the root mean squared error (RMSE) is used in connection with the MSE, as the RMSE gives the magnitude of the errors on the original scale. 

Similarly as the errors pertaining to model fitting, the forecast errors, $e_t(1)$, should also (ideally) be uncorrelated, normally distributed, and have constant variance. In the statistical literature, such forecast errors are referred to as Gaussian white noise \cite{Montgomery2015}. 
