...
In \vref{fig:time_series_plots}, a notable reduction in NO$_2$ levels can be seen during 2020 and early 2021. This reduction is most likely due to the COVID-19 pandemic, and by late 2021, pre-pandemic NO$_2$ levels are again approached.  Because of this, a train-test split (see below) was done to entirely avoid using the data for 2020.
...
Initially, a simple linear regression model was fit with OLS where the response variable at lag one was used as predictor. This model did not have any significant autocorrelation, but when also including the response variable at lag two as predictor, the Durbin-Watson statistic improved (i.e., was brought closer to 2). Including additional response variables after the first two lags did not lead to further improvements in terms of eliminating autocorrelation, however, the response variable at lag 24 turned out to also be significant. 

It should be noted that a log transformation of the response as well as input variables were required to stabilize the variance of the residuals, and also bring them closer to a normal distribution. Even so, deviation from normality was indicated, as can be seen in the residual plots in \cref{fig:residuals_MLR_PM10} in appendix \ref{chapt:appendix_B}. For inference, this is not optimal, however, by the central limit theorem, confidence intervals, t-tests, etc.\ are ensured to be increasingly accurate as the sample size increases, even if the distribution of the residuals is not normal \cite{Montgomery2012}. For the problem at hand, given the large amount of data points available, and the fact that the deviation from normality was not severe, this issue was regarded as largely unproblematic. (A more thorough interpretation of the residual plots is given in appendix \ref{chapt:appendix_B}.)
